...
'''  for trignomertic function [sin(3**2),cos()] for logarithrmic function [log(x); eq = log10(x)-1 => log(x,10); eq =ln(x)-2ln(x-1) => log(x) - 2*log(x - 1) ] 
            ...
            e-x *(3.2sin(x)-0.5cos(x)) =>exp(-x) * (3.2 * sin(x) - 0.5 * cos(x)) '''
...
try:
    ...
    with open("RephsonMethod.txt",'a') as info:
        info.write(f"The root is for Equation : [{equation}] is : {round(root,7)} . With initial guess is : [{initial_guess}]")
        print("\n")
        info.write("\n")
...
# print("Newton-Raphson method")
# print("\n")
# # Example input for the given function:
# equation = "exp(-x) * (3.2 * sin(x) - 0.5 * cos(x))"
# initial_guess = 3  # A reasonable initial guess
